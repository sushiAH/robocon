...
def set_goal_vel_callback(data):
    print("Set Goal Velocity of ID %s = %s" % (data.id, data.velocity)) 
    dxl_comm_result, dxl_error = packetHandler.write4ByteTxRx(portHandler, data.id, ADDR_GOAL_VELOCITY, data.velocity)
...
def get_present_vel(req):    
    dxl_present_velocity, dxl_comm_result, dxl_error = packetHandler.read4ByteTxRx(portHandler, req.id, ADDR_PRESENT_VELOCITY)
    print("Present Velocity of ID %s = %s" % (req.id,dxl_present_velocity))
...
def read_write_py_node():
    ...
    rospy.Subscriber('set_velocity', SetVelocity,set_goal_vel_callback)
    rospy.Service('get_position', GetPosition, get_present_pos)
    rospy.Service("get_velocity",Getvelocity, get_present_vel)
    rospy.spin()
...
def main():
    ...
    present_mode1 =  packetHandler.read1ByteTxRx(portHandler,DXL_ID1,ADDR_OperatingMode)
     
    present_mode2 =  packetHandler.read1ByteTxRx(portHandler,DXL_ID2,ADDR_OperatingMode)
